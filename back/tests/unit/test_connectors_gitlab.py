...
from tuesmon_contrib_gitlab_auth import connector as gitlab


def test_url_builder():
    with patch("tuesmon_contrib_gitlab_auth.connector.URL", "http://localhost:4321"):
...
def test_login_without_settings_params():
    with pytest.raises(gitlab.GitLabApiError) as e, \
            patch("tuesmon_contrib_gitlab_auth.connector.requests") as m_requests:
...
def test_login_success():
    with patch("tuesmon_contrib_gitlab_auth.connector.requests") as m_requests, \
            patch("tuesmon_contrib_gitlab_auth.connector.URL", "http://localhost:4321"), \
            patch("tuesmon_contrib_gitlab_auth.connector.CLIENT_ID", "*CLIENT_ID*"), \
            patch("tuesmon_contrib_gitlab_auth.connector.CLIENT_SECRET", "*CLIENT_SECRET*"):
        m_requests.post.return_value = m_response = Mock()
        m_response.status_code = 200
        m_response.json.return_value = {"access_token": "xxxxxxxx"}
...
def test_me():
    with patch("tuesmon_contrib_gitlab_auth.connector.login") as m_login, \
            patch("tuesmon_contrib_gitlab_auth.connector.get_user_profile") as m_get_user_profile:
